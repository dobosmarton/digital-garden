...
tags:
  ...
  - rag
  - ai
description: "One of the best use case of AI and RAG is to extract information from large amount of data. In this article we build a simple web app that allows us to upload documents and ask questions about the uploaded documents."
status: published
---
...
You need to have installed Python and Poetry in your machine.

[Poetry](https://python-poetry.org/) is a dependency management and packaging tool for Python that simplifies the process of creating and managing Python projects. It allows developers to declare and manage project dependencies in a straightforward manner, using a `pyproject.toml` file. Poetry handles virtual environments automatically, ensuring that dependencies are isolated and consistent across different environments.

You also need an [OpenAI account to generate an API key](https://platform.openai.com/docs/quickstart/create-and-export-an-api-key). When you have that, create a .env file based on the .env.example and paste the key there. The Minio bucket name and keys can be generated once you started Minio based on the docker-compose file in the next section.
...
You can use the reload flag if you want to modify the code, so the app will reload every time you change something in the codebase. If you see this messages in your terminal then everything is fine, you can use the API to upload some documents.
...
You can validate the result by opening the web interface of Minio and check the uploaded file in the right bucket. After a successful upload, finally you can unlock the model’s intelligence and ask questions.
...
The [main.py](http://main.py) file stores the FastAPI routes and a document index context. It is a dependable that we can inject into our routes to provide access to the document index.
...
Document index is actually a VectorStoreIndex from [LlamaIndex](https://docs.llamaindex.ai/en/stable/).
...
Our two type of data stores serves different needs. When you upload a file, we store it immediately to Minio, if later we need to build the index from scratch, we have every resources to do so. Minio is an S3 compatible document store, if you want to deploy the application to the cloud, you can easily switch Minio to S3.
...
The storage context is a LlamaIndex utility container for storing nodes, indices, and vectors. Using storage context makes easier to run queries against the ML model. We persist the context in our S3 compatible Minio storage.
...
Finally, the `query` function allows us to perform searches against our indexed documents, leveraging the power of the VectorStoreIndex to find relevant information based on the user's query.
...
This simple RAG application demonstrates the power of combining document storage, vector embeddings, and natural language processing to create an intelligent question-answering system. By using tools like LlamaIndex, ChromaDB, and Minio, we've built a flexible solution that can be easily adapted for various use cases. You can easily extend the general knowledge base with you private documents without publishing any sensitive data to the internet. As AI technologies continue to evolve, applications like this will become increasingly valuable for extracting insights from large volumes of unstructured data.
