...
tools:
  - slug: react-native-bip39
    note: "BIP39 mnemonic generation the custom solution is based on"
  - slug: libauth
    note: "BIP32 HD key derivation from seed → private keys"
  - slug: react-native-keychain
    note: "Secure key storage in iOS Keychain / Android, no hardcoding"
  - slug: web3js
    note: "Private key → account + balance via Infura RPC"
  - slug: infura
    note: "The RPC provider the wallet talks to the chain through"
  - slug: react-navigation
    note: "Conditional login flow based on wallet state"
  - slug: react-native-reanimated
    note: "Native-driver animated tab bar"
status: published
series:
  order: 1
