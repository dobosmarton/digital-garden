...
tools:
  - slug: ultralytics-yolo
    note: 'The 60-class entity detector that gives the agent its "eyes"'
  - slug: pyautogui
    note: "Sends mouse clicks and keypresses to the game window"
  - slug: claude-sonnet
    note: "The model both the strategist and executor run on"
  - slug: anthropic-structured-outputs
    note: "The single fast executor call on routine turns"
  - slug: anthropic-prompt-caching
    note: "`cache_control: ephemeral`, the 90% cost cut the economics depend on"
status: published
series:
  order: 1
...
A quick note on terminology: AoE2 is a real-time strategy game, not a turn-based one — the game clock never stops. But the agent has to act in discrete steps, so I impose an artificial "turn" on top of the real-time loop: a tick where the pipeline captures the screen, runs perception, and lets the LLM decide on actions. There's a small fixed delay between ticks (0.3s), but in practice the LLM call dominates each turn's wall-clock. The game keeps running between turns; the agent just samples it. Throughout this series, "turn" always means one of these agent decision cycles, not a pause in the game.
...
**Executor**: Receives text only — the YOLO entity list, the strategist's goals, recent memory, game knowledge. Runs every turn. Routine turns resolve in a single fast structured-output call; combat and housing emergencies fall back to an agentic tool-use loop (up to 7 tool calls). A per-call `effort` setting (default `low`) keeps the common case quick.

Both tiers run the same underlying model — `claude-sonnet-4-6` (the executor started on Haiku and moved to Sonnet for more reliable instruction-following). The split is about separating expensive vision from cheap text reasoning, and running them at different frequencies.
...
- **Block 2** (~600 tokens): age-specific prompt (`dark.md`, `feudal.md`, etc.). Cached too — it changes only ~3 times per game when the agent ages up, so within an age it's a cache hit every turn.

The dynamic per-turn context — entity list, resources, recent turns — is not cached, because it changes every turn. That part is small. (On combat turns that run the tool loop, a moving cache breakpoint keeps the growing conversation cached across iterations — more in Episode 5.)
...
Multiplayer is not attempted. Turn latency varies by path — routine turns resolve in a few seconds (a single executor call plus the 0.3s loop delay), but combat turns that fall back to the full tool loop can still reach ~20–30 seconds. Either way it's LLM-bound, not game-loop-bound, and that slow tail is wrong for real-time PvP.
